...
Each page's script is generated by a one-shot ``llm.query`` call — the prompt
carries the full page snapshot + profile + completed-page list, so there is no
per-job conversation to hold.
"""
...
class ScriptApplyEngine:
    ...
    Each page's script is generated by a one-shot ``llm.query`` call (Claude
    Agent SDK, subscription auth) — the prompt is fully self-contained.
    """
...
Output the completed script only — no markdown, no explanation."""
    ...
    async def _call_llm(self, prompt: str, page_url: str, element_count: int) -> str | None:
        ...
        try:
            script = await llm.query(prompt, model=self.model, timeout=180)
