...
| 5 — Phase 4 | **T16b** — decompose `_llm_guided_apply` on the Agent SDK (primary OffsiteApply path) + retire `ScriptApplyEngine` | **2 PRs.** PR 1 (`ScriptApplyEngine` retired + `page_snapshot`/`decide_action`/`detect_terminal_state`/`handle_auth` seams) ✅ merged (#30). PR 2 (`execute_action` + `_StepState` + thin orchestrator) — PR #31, approved, pending merge. |
...
**Status:** PR 1 ✅ **merged (#30)**. PR 2 — **PR #31, approved, pending merge**. Split into two PRs because a single behaviour-preserving diff over ~1,500 lines of the primary production path was too large to review safely.
