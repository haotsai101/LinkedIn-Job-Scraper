...
**Phase:** T38 follow-up · **Risk:** trivial · **Status:** ✅ **CLOSED** (#66, 2026-09-09) · **Sev:** P4 · Found while verifying T17 (2026-09-09).
...
**Live QA run 1 (2026-09-11, 2 real Workday jobs — BECU Sr Data Engineer, Alteryx Sr Software Engineer):** domain unblock confirmed working (both jobs reached the real Workday application flow instead of instant-blocking); `_prefer_workday_autofill` correctly detected and clicked "Autofill with Resume" on both. Two findings, both now filed as **T52** (blocking — found the root cause) and a smaller note below:
- The `autofillWithResume` landing page's resume-upload control wasn't surfaced by the snapshot walk (no form fields detected there) — the LLM self-corrected by trying "Apply Manually" instead of getting stuck, which is good recovery behavior, but means Autofill's actual value (Workday pre-filling from the parsed resume) never engaged on either job. Worth its own follow-up once T52 unblocks the account wall enough to reach this step reliably and a real DOM capture is possible.
- **Root cause of the blocked outcome on both jobs — T52**: `_handle_auth_page` and `_handle_auth`'s `phase="form"` branch both go straight to `"blocked"` when no stored credentials exist for a domain — **`_try_register` (the account-auto-creation function, fully built and already used by T51's new Workday selectors) has zero call sites anywhere in the file.** This is a pre-existing gap (not introduced by T51) that blocks account auto-creation for every ATS requiring one, not just Workday. T51's own live QA is what surfaced it. T51 stays open pending T52.

**Live QA run 2 (2026-09-11, same 2 jobs, after T52 merged):** confirmed T52's wiring works — both jobs now log `"attempting registration"` and BECU's log shows `"[Auth] Navigated to registration via 'button:has-text(\"Create account\")'"`, proving `_try_register` is genuinely invoked and finds a real Create-account control on live Workday. **New finding:** both registrations then log `"[Auth] Registration appears to have failed — not saving credentials"` — `_try_register`'s success check (`url_changed or any success-phrase in body`) didn't fire, so it correctly declined to save bad/incomplete credentials and fell through to `"blocked"` rather than a false success. No account was created/saved (confirmed no `created_accounts.json` write). **Not yet diagnosed why** — `_try_register` takes no screenshot at its own failure point, so there's no visual evidence of what the actual submitted-but-unrecognized-outcome page looked like (wrong/missing required field the Workday form validation is blocking on, a success state whose text doesn't match `_success_phrases`, or a genuine submit failure). Needs a screenshot-on-registration-failure diagnostic before the next live QA round can make progress past this point. Filed as **T53**.

---

## T52 — Account auto-registration (`_try_register`) is fully built but never called

**Phase:** apply-quality (blocks T51 live QA) · **Risk:** medium (creates real external accounts under the applicant's real email) · **Status:** ✅ **CLOSED** (#75, `328119d`, 2026-09-11 — 466 tests, reviewer confirmed the `self.inbox` gate is airtight and the shared per-job guard is correct) · **Sev:** P1 · Found 2026-09-11 during T51's live Workday QA (2/2 jobs blocked at the account wall for this reason).

**Verified live (2026-09-11, round 2):** the wiring works — both re-run jobs logged `"attempting registration"`, and BECU's log confirms `_try_register` found and clicked a real `'button:has-text("Create account")'` on live Workday. The registration attempt itself doesn't yet succeed (→ **T53**), but the wiring this ticket exists for — reaching `_try_register` at all — is proven working end-to-end against a real ATS.

**Root cause:** `OffsiteApplyFlow._handle_auth_page` (`linkedin_apply.py:5913`)'s docstring says it "falls back to registering a new account," but the function body never does — it checks `_find_account_for_domain(domain)`, and if nothing is found, goes straight to `return "blocked"`. Same gap in `_handle_auth`'s `phase="form"` mid-loop password-wall branch (`linkedin_apply.py:~4229-4245`) — the one that actually fired in the T51 live QA run (a Workday password field appeared mid-flow, not on a URL matching `_LOGIN_PATHS`). **`_try_register` (`linkedin_apply.py:6019`) is fully implemented** — clicks a registration link if needed, fills the form (`_fill_registration_form`, with T51's new Workday-specific selectors), submits, detects success/error via body-text heuristics, follows an email-verification link via `self.inbox.wait_for_link(domain)` when required, and saves credentials via `_append_account`/`created_accounts.json` — but has **zero call sites** anywhere in the file. Confirmed via `grep -n "self\._try_register(" linkedin_apply.py` → no matches outside its own `def`.

**Impact:** account auto-creation has never worked for ANY offsite ATS requiring a new account (not Workday-specific) — every such job has always gone straight to `-3` blocked instead of attempting registration. T51's Workday-specific `_try_register`/`_fill_registration_form` selectors are correctly built and tested but unreachable until this is fixed.

**Fix:**
1. In `_handle_auth_page`, after the `existing = _find_account_for_domain(domain)` / login-attempt branch, when no existing account is found: if `self.inbox` is available (email verification capability exists — gate registration attempts on this so a signup requiring verification doesn't silently strand a half-created, unusable account), call `await self._try_register(page, domain)`. On success return `True` (same contract as a successful login); on failure, log and `return "blocked"` (preserve the existing no-account-and-can't-make-one → blocked semantics).
2. Same wiring in `_handle_auth`'s `phase="form"` branch, in the `existing` `else` path (currently goes straight to the "Login wall detected... blocked" print).
3. **New per-job guard** (mirroring the existing `self._auth_attempted` pattern, reset alongside it at the top of `_llm_guided_apply`) — e.g. `self._registration_attempted` — set `True` the first time `_try_register` is invoked in either call site, and checked before attempting again, so a stuck/failed registration can't be retried every step-loop iteration (the `phase="form"` branch currently has NO such guard at all, unlike the url-phase branch's existing `_auth_attempted` check — this is what would make an unguarded registration attempt loop-prone).
4. Verify `_try_register`'s success detection doesn't false-positive on a page that merely didn't error (its `_success_phrases`/`_error_phrases` body-text heuristic) — not a T52 change, just confirm it during testing.
5. Tests: extend `tests/test_workday_flow.py` or a new file with a synthetic "no stored credentials, registration form appears, fill + submit succeeds" fixture through both call sites (url-phase and form-phase), a guard-flag regression test (second password-field sighting in the same job doesn't re-attempt registration), and a "registration fails → blocked, not an infinite retry" case.
6. Re-run T51's live QA (the 2 Workday jobs already re-queued once — reset again, or pick 2 fresh ones from the blocked pool) once this merges, to see whether the account wall now actually gets past.

**Do NOT** silently create accounts when `self.inbox` is unset and the site requires email verification — that strands a real signup the applicant has no way to complete. Gate on capability, not on best-effort.

---

## T53 — `_try_register` reaches a real Workday create-account form but doesn't complete successfully

**Phase:** apply-quality (T51/T52 follow-up) · **Risk:** low (diagnostic gap, no bad data written) · **Status:** 🔜 **OPEN** · **Sev:** P2 · Found 2026-09-11, live QA round 2 (BECU, Alteryx — same 2 jobs as T51/T52's runs).

**Symptom:** with T52 merged, both jobs correctly reach `_try_register`, which correctly finds and clicks a real `Create account` control (BECU: `button:has-text("Create account")`), fills the registration form, and submits — but then logs `"[Auth] Registration appears to have failed — not saving credentials"` on both. No credentials were saved (confirmed no `created_accounts.json` write) and the job correctly resolved to `-3` blocked rather than a false "applied" or a bad-data save — `_try_register`'s conservative success check (`url_changed or any(_success_phrases in body)`, no match → don't save) worked exactly as designed here. The problem is purely that registration itself isn't completing, not that a failure was mishandled.

**Not yet diagnosed why** — `_try_register` takes no screenshot at its own failure point (screenshots are only taken by the outer step loop, once per LLM-decision step; the registration attempt happens *inside* one step, between two screenshots, so nothing captures the intermediate state). Candidates, unconfirmed:
- A required field on Workday's real create-account form isn't being filled (only email/password/verifyPassword/T&Cs-checkbox are handled by `_fill_registration_form` — Workday forms sometimes also require first/last name or a security question on this specific page, separate from the "My Information" step later in the flow).
- The submit click chain's first-tried selector (`[data-automation-id="click_filter"]`, the Workday overlay) succeeds and the loop `break`s before ever clicking the real `createAccountSubmitButton` — worth checking whether that ordering (overlay-first) is actually correct or is itself the bug (pre-dates T51/T52, not introduced by them, but worth a look here since it's directly upstream of this symptom).
- Workday's actual post-submit success page uses wording not in `_success_phrases` (`"verify your email"`, `"check your email"`, `"account created"`, `"registration complete"`, `"welcome"`, `"thank you for registering"`, `"successfully created"`, `"sign in"`, `"log in"`) — e.g. if it just silently advances to "My Information" step 2 of the wizard with no confirmation text at all, `url_changed` should still catch it unless the URL is unchanged (possible on a SPA).

**Fix (once diagnosed):**
1. Add a screenshot capture (reuse the existing `--verbose` screenshot helper) immediately after `_try_register`'s submit-click attempt and immediately before the success/failure check, so the next live QA round can see exactly what page is showing.
2. Re-run live QA against the same 2 jobs (or fresh ones from the blocked pool) with `--verbose`, inspect the new screenshot, and fix whichever of the candidates above (or something else the screenshot reveals) is the actual cause.
