...
| T40 | P3 | ✅ **fixed (PR pending)** — (found by the T22/T38 QA run 2026-09-06) `_get_profile_value`'s `"years of experience"` catch-all ran *before* the T32 tiered "years of &lt;skill/role&gt;" branch, so `"How many years of experience do you have as a Lead?"` returned full `years_experience` (`4`) — asserted a whole career in a role the applicant has never held. Fix: new `_years_label_names_a_role_or_skill()` guard — the catch-alls (both the `"years of experience"` one and the `"total / relevant experience"` one) now fire only for *bare* overall-experience phrasings ("years of professional / work / total experience"); any `"…as a &lt;role&gt;"` / `"…experience with/in &lt;skill/domain&gt;"` phrasing falls through to the T32 tiered branch. |
