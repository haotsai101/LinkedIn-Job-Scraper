...
| T47 | P3 | (tech debt — raised by the T43 #56 reviewer) `_get_profile_value` has grown to ~30 sequential `if` branches where every new rule (T40/T41/T42/T43) must reason about global ordering to avoid an earlier branch stealing a label. Convert to an ordered list of `(matcher, resolver)` pairs — each rule self-contained, ordering explicit in one place, testable in isolation. Non-urgent; do it before the next `_get_profile_value` change, or when the branch count next bites. See `## T47` below. **Not started.** |
| T48 | P4 | (T42 follow-up — from the delayed original #45 reviewer) `_anchored_in_bg`'s lookaround class omits `-`, so 2-char skill tokens match hyphen fragments (`go` in `"go-to"`, `co` in `"co-founder"`); T42's `len(w)>=2` widened the exposure. Bounded (function errs toward "keep full figure"). Also: string-form `skills` split doesn't strip a leading `"and "` → `"Python, Go, and R"` → `"and r"` misses the step-3(b) exact match. Fixes are one-liners; fold into T47 or a quick PR. See `## T48` below. **Not started.** |
| T49 | P4 | (found verifying T17, 2026-09-09) `apply_jobs.py:58` hard-imports `openai` at module level, but post-T38 `OpenAI` is only used by the legacy `--setup` interview (which builds the client lazily anyway). The whole apply agent fails to import without `openai` for a near-unused path; `nim_client.py` already soft-imports it. Move the import into `build_profile_interactively`; consider dropping `openai` to `[optional-dependencies]`. See `## T49` below. **Not started.** |
...
**Phase:** T42 follow-up · **Risk:** low · **Status:** open · **Sev:** P4 · Raised by the (very-delayed) original T42 (PR #45) reviewer — same "approved" verdict as the replacement reviewer, two extra latent nits.
...
**Phase:** T38 follow-up · **Risk:** trivial · **Status:** open · **Sev:** P4 · Found while verifying T17 (2026-09-09).
