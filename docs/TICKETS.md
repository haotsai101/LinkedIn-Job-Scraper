...
| T47 | P3 | ✅ **fixed (PR pending)** — (tech debt — raised by the T43 #56 reviewer) `_get_profile_value` had grown to ~80 sequential `if` branches where every new rule (T40/T41/T42/T43) had to reason about global ordering to avoid an earlier branch stealing a label. Converted to a module-level ordered `_PROFILE_VALUE_RULES` list of `_ProfileRule(name, matches, resolve)` entries iterated once (first match wins). Normalization preamble unchanged; behaviour-preserving — `tests/test_profile_value.py` stays 100% green with zero assertion changes. See `## T47` below. |
| T48 | P4 | ✅ **fixed (PR pending)** — (T42 follow-up — from the delayed original #45 reviewer) folded into T47. `_anchored_in_bg`'s lookaround class now includes `-` and `_` (`(?<![a-z0-9+#.\-_])` … `(?![a-z0-9+#.\-_])`) so a 2-char skill token can't match a hyphen fragment (`go` in `"go-to"`, `ai` in `"ai-driven"`); verified not to regress `c` / `c++` / `c#` / `node.js` anchoring. New `_split_skill_string` strips a leading `"and "` from string-form `skills` entries (`"Python, Go, and R"` → `["Python", "Go", "R"]`) so the step-3(b) exact single-char match finds `"r"`. See `## T48` below. |
...
**Phase:** T43 follow-up · **Risk:** low (pure refactor, characterization tests already dense) · **Status:** ✅ **fixed (PR pending)** — folds in T48 · **Sev:** P3 · Raised by the T43 (PR #56) reviewer.
...
**Phase:** T42 follow-up · **Risk:** low · **Status:** ✅ **fixed (PR pending)** — folded into T47 · **Sev:** P4 · Raised by the (very-delayed) original T42 (PR #45) reviewer — same "approved" verdict as the replacement reviewer, two extra latent nits.

**Resolution:** (1) `_anchored_in_bg`'s lookarounds are now `(?<![a-z0-9+#.\-_])` … `(?![a-z0-9+#.\-_])` — a 2-char token can no longer match across a hyphen (`go` in `"go-to"`, `ai` in `"ai-driven"`). Regression-tested against `c` / `c++` / `c#` / `node.js` / bare `c` anchoring (`test_anchoring_still_recognises_real_skill_tokens`). (2) New module-level `_split_skill_string(raw)` splits on `,` / `;` and strips a leading `"and "` per entry; used by both `_years_label_names_a_foreign_role_or_skill` (the step-3(b) exact single-char check — the ticket's target) and the T32 tiered resolver in `_get_profile_value` (same latent bug, negligible effect on tested paths, changed for consistency). The data fix (add `"Machine Learning"` / `"ML"` to `user_profile.json`) is **not** done here — code-only ticket.
